...
window.fetch=async(...args)=>{
  ...
  if(requestUrl===RE_AUTH_API){response.clone().json().then(data=>{if(data?.ok&&data?.session?.token){window.mmSessionToken=data.session.token;window.mmSessionExpiresAt=new Date(data.session.expires_at).getTime();window.dispatchEvent(new CustomEvent('make-money-authenticated'));}}).catch(()=>{});}
  return response;
};
let realEstateBusy=false,realEstateTimer=null,realEstateItems=[];
...
function reSetBalance(value){const el=document.getElementById('balance');if(el)el.textContent=reFormatMM(value);window.mmBalance=Number(value||0);}
...
function reRender(){
  ...
  const currentBalance=Number(window.mmBalance||0);
  for(const item of realEstateItems){
    const row=document.createElement('div');
    const ownedNow=Number(item.owned_units||0)>0;
    const affordable=!ownedNow&&currentBalance>=Number(item.price||0);
    row.className=`property-row${!ownedNow&&!affordable?' locked':''}`;
    ...
    const owned=document.createElement('span');owned.textContent=ownedNow?'Owned':(!affordable?'Locked':'Not owned');
    meta.append(price,income,owned);row.append(head,meta);
    const button=document.createElement('button');button.type='button';
    if(!ownedNow){button.textContent=affordable?`Buy · ${reFormatMM(item.price)} MM`:`Locked · ${reFormatMM(item.price)} MM`;button.disabled=realEstateBusy||!affordable;if(!affordable)button.className='secondary';else button.onclick=()=>reAction(item,'buy');}
...
async function reLoad(){
  if(!reSessionValid())return;const status=document.getElementById('realEstateStatus');status.textContent='Loading properties securely...';
  try{const r=await reOriginalFetch(REAL_ESTATE_API,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${window.mmSessionToken}`},body:JSON.stringify({action:'list'}),cache:'no-store'});const data=await r.json().catch(()=>null);if(!r.ok||!data?.ok)throw new Error(data?.error||`HTTP ${r.status}`);realEstateItems=Array.isArray(data.properties)?data.properties.map(p=>({...p,price:Number(p.price||0),income_per_day:Number(p.income_per_day||0),owned_units:Number(p.owned_units||0)})):[];status.textContent=realEstateItems.length?'Buy one property and collect its income every 24 hours.':'No properties available.';reRender();}
...
async function reAction(item,action){
  if(realEstateBusy||!reSessionValid())return;realEstateBusy=true;reRender();const status=document.getElementById('realEstateStatus');status.textContent=action==='buy'?`Buying ${item.name} securely...`:`Claiming ${item.name} income securely...`;
  try{const r=await reOriginalFetch(REAL_ESTATE_API,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${window.mmSessionToken}`},body:JSON.stringify({action,property_id:item.property_id,operation_key:reOperationKey()}),cache:'no-store'});const data=await r.json().catch(()=>null);if(!r.ok||!data?.ok)throw new Error(data?.error||`HTTP ${r.status}`);reSetBalance(data.balance);status.textContent=action==='buy'?`${item.name} purchased.`:`Income credited: +${reFormatMM(data.reward)} MM.`;await reLoad();}
  catch(e){const messages={INSUFFICIENT_BALANCE:'Not enough MM.',ALREADY_OWNED:'You already own this property.',PROPERTY_NOT_OWNED:'You do not own this property.',PROPERTY_COOLDOWN:'Income is not ready yet.',PROPERTY_CLOCK_INVALID:'Property timer is invalid. Try again later.',OPERATION_KEY_REUSED:'This operation was already processed.',SESSION_EXPIRED:'Session expired. Reopen the Mini App.',RATE_LIMITED:'Too many requests. Try again in a minute.'};status.textContent=messages[e.message]||`Real Estate action failed (${e.message}).`;}
