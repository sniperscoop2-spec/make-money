...
function reSetBalance(value){if(typeof window.mmSetBalance==='function')window.mmSetBalance(value);}
function reClearTimer(){if(realEstateTimer){clearTimeout(realEstateTimer);realEstateTimer=null;}}
function reSchedule(item){reClearTimer();const dates=realEstateItems.map(x=>x.next_claim_at).filter(Boolean).map(x=>new Date(x).getTime()).filter(Number.isFinite);if(!dates.length)return;const next=Math.min(...dates);realEstateTimer=setTimeout(()=>reRender(),Math.max(1000,next-Date.now()+100));}
...
function reRender(){
  ...
  for(const item of realEstateItems){
    ...
    meta.append(price,income,owned);
...
async function reLoad(){
  ...
  const status=document.getElementById('realEstateStatus');
  status.textContent='Loading properties securely...';
  try{
    const r=await fetch(REAL_ESTATE_API,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${window.mmSessionToken}`},body:JSON.stringify({action:'list'}),cache:'no-store'});
    ...
    status.textContent=realEstateItems.length?'Buy one property and collect its income every 24 hours.':'No properties available.';
    reRender();
...
async function reAction(item,action){
  ...
  try{
    const r=await fetch(REAL_ESTATE_API,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${window.mmSessionToken}`},body:JSON.stringify({action,property_id:item.property_id,operation_key:reOperationKey()}),cache:'no-store'});
    const data=await r.json().catch(()=>null);if(!r.ok||!data?.ok)throw new Error(data?.error||`HTTP ${r.status}`);
    reSetBalance(data.balance);
    status.textContent=action==='buy'?`${item.name} purchased.`:`Income credited: +${reFormatMM(data.reward)} MM.`;
    await reLoad();
  }catch(e){
    const messages={INSUFFICIENT_BALANCE:'Not enough MM.',ALREADY_OWNED:'You already own this property.',PROPERTY_NOT_OWNED:'You do not own this property.',PROPERTY_COOLDOWN:'Income is not ready yet.',PROPERTY_CLOCK_INVALID:'Property timer is invalid. Try again later.',OPERATION_REUSED:'This operation was already processed.',SESSION_EXPIRED:'Session expired. Reopen the Mini App.',RATE_LIMITED:'Too many requests. Try again in a minute.'};
