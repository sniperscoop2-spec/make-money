-- Fix: the interim (not-yet-finished) response after split/double reported
-- the balance read at function entry, before that action's own deduction
-- was applied -- so the client briefly displayed an overstated balance
-- (the real make_money_players.balance was always correct; only the
-- returned JSON lagged by one action). Re-read balance right before
-- building the interim response.
