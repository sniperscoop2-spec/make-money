...
declare
  ...
  v_reward numeric(20,0);
  v_completed boolean;
  v_new_balance numeric(30,4);
...
begin
  ...
  where s.token_hash=p_session_hash and s.revoked_at is null and s.expires_at>now()
  limit 1;
  ...
  select c.reward into v_reward
  from public.make_money_achievement_catalog c
  where c.achievement_id=p_achievement_id and c.active;
  if v_reward is null then raise exception 'achievement_not_found'; end if;

  select a.claimed_at into v_claimed_at
  from public.make_money_achievements a
  where a.player_id=v_player and a.achievement_id=p_achievement_id;
  if v_claimed_at is not null then raise exception 'achievement_already_claimed'; end if;

  select p.balance into v_balance
  from public.make_money_players p
  where p.id=v_player
  for update;
  if v_balance is null then raise exception 'invalid_session'; end if;
  ...
  v_completed:=case p_achievement_id
    ...
    when 'first_casino' then exists(select 1 from public.make_money_casino_spins s where s.player_id=v_player)
  ...
  insert into public.make_money_idempotency(player_id,operation_key,operation_type,amount)
  values(v_player,p_operation_key,'achievement_claim',v_reward)
  on conflict(player_id,operation_key) do nothing;
  if not found then raise exception 'operation_reused'; end if;

  insert into public.make_money_achievements(player_id,achievement_id)
  values(v_player,p_achievement_id)
  on conflict do nothing;
  if not found then raise exception 'achievement_already_claimed'; end if;

  v_new_balance:=v_balance+v_reward;
  update public.make_money_players p set balance=v_new_balance,updated_at=now() where p.id=v_player;
  ...
  insert into public.make_money_transactions(player_id,amount,type,reference,metadata,balance_before,balance_after)
  values(v_player,v_reward,'achievement_reward',p_operation_key,jsonb_build_object('achievement_id',p_achievement_id),v_balance,v_new_balance);
