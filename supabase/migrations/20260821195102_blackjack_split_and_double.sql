-- Blackjack: split and double.
alter table public.make_money_casino_blackjack_rounds
  add column if not exists is_split boolean not null default false,
  add column if not exists active_hand smallint not null default 1,
  add column if not exists hand1_doubled boolean not null default false,
  add column if not exists hand2_cards jsonb,
  add column if not exists hand2_status text,
  add column if not exists hand2_bet numeric(20,4),
  add column if not exists hand2_doubled boolean not null default false,
  add column if not exists hand2_payout numeric(20,4) not null default 0;
...
declare
  ...
  is_split boolean; active_hand smallint;
...
begin
  ...
  is_split := coalesce(r.is_split,false);
  active_hand := coalesce(r.active_hand,1);
  h1_status := r.status;
  ...
  if (not is_split and h1_status<>'active')
     or (is_split and h1_status<>'active' and (h2_status is null or h2_status<>'active')) then
  ...
  if p_action='split' then
    if is_split then raise exception 'blackjack_split_not_allowed'; end if;
    ...
    if (r.player_cards->0->>'rank')='A' then
      ...
      h2_status := 'active';
      active_hand := case when h1_status='active' then 1 else 2 end;
    end if;

    update make_money_casino_blackjack_rounds set
      player_cards=pc, dealer_cards=dc, hand2_cards=hc2, is_split=true, active_hand=active_hand,
      status=h1_status, hand2_status=h2_status, hand2_bet=h2_bet, balance_after=newbal
    where id=r.id;

  elsif p_action='double' then
    if active_hand=1 then
      ...
      newbal := oldbal - extra;
      update make_money_players set balance=newbal, updated_at=now() where id=pid;
      if is_split and h1_status<>'active' and h2_status='active' then active_hand:=2; end if;
      update make_money_casino_blackjack_rounds set player_cards=pc, bet=h1_bet, hand1_doubled=true, status=h1_status, active_hand=active_hand, balance_after=newbal where id=r.id;
    else
    ...
    if active_hand=1 then
      ...
      if is_split and h1_status<>'active' and h2_status='active' then active_hand:=2; end if;
      update make_money_casino_blackjack_rounds set player_cards=pc, status=h1_status, active_hand=active_hand where id=r.id;
    else
    ...
    if active_hand=1 then
      if h1_status<>'active' then raise exception 'blackjack_round_finished'; end if;
      h1_status:='standing';
      if is_split and h2_status='active' then active_hand:=2; end if;
      update make_money_casino_blackjack_rounds set status=h1_status, active_hand=active_hand where id=r.id;
    else
      if h2_status<>'active' then raise exception 'blackjack_round_finished'; end if;
      h2_status:='standing';
  ...
  round_finished := (not is_split and h1_status<>'active')
     or (is_split and h1_status<>'active' and h2_status<>'active');
  ...
  if round_finished then
    ...
    if h1_status='lost' then
      ...
      pv := make_money_casino_blackjack_value(pc);
      if not is_split and pv=21 and jsonb_array_length(pc)=2 and dv<>21 then
    ...
    response := jsonb_build_object(
      ...
      'is_split',is_split,'active_hand',active_hand,
