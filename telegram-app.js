...
async function claimMining(){
  if(!sessionToken||Date.now()>=sessionExpiresAt){$('miningStatus').textContent='Session expired. Reopen the Mini App.';return;}
  ...
  }catch(e){$('miningStatus').textContent=e.message==='Failed to fetch'?'Mining server unavailable.':`Claim failed (${e.message}).`;button.disabled=false;}
}
async function start(){
  ...
  try{
    ...
    const p=data.player;
    $('statusCard').hidden=true;$('profile').hidden=false;$('mining').hidden=false;
    ...
    updateMiningTime(p.next_claim_at);
