...
let sessionToken=null,sessionExpiresAt=0,balance=0,jobs=[],jobsBusy=false,jobsTimer=null,jobsProgressTimer=null,jobProgress=null,rankingTimer=null;
...
function renderJobProgress(active,box){if(!box||!active)return;const progress=jobProgress?.job_id===active.job_id?jobProgress:null;const cooldown=Math.max(3600,Number(progress?.cooldown_seconds||active.cooldown_seconds||3600));const last=progress?.last_claim_at?new Date(progress.last_claim_at).getTime():Date.now();const target=progress?.next_claim_at?new Date(progress.next_claim_at).getTime():last+cooldown*1000;const available=Number(progress?.available_salary||0);const percent=Math.max(0,Math.min(100,Number(progress?.progress_percent||0)));window.mmJobsReady=available>0;const wrap=document.createElement('div');wrap.className='job-salary-progress';wrap.style.cssText='margin-top:12px;padding:12px;border-radius:12px;background:#0a1019;border:1px solid rgba(255,255,255,.06);';const head=document.createElement('div');head.style.cssText='display:flex;justify-content:space-between;align-items:center;gap:10px;font-size:14px;';const label=document.createElement('strong');label.textContent='Salary progress';const value=document.createElement('span');value.style.fontWeight='700';value.style.color='#63e69a';head.append(label,value);const track=document.createElement('div');track.style.cssText='height:8px;margin-top:9px;border-radius:999px;background:#18212d;overflow:hidden;';const fill=document.createElement('div');fill.style.cssText=`height:100%;width:${percent}%;border-radius:999px;background:#63e69a;transition:width .25s linear;`;track.append(fill);const next=document.createElement('small');next.style.cssText='display:block;margin-top:7px;color:#9da9ba;';wrap.append(head,track,next);const claim=document.createElement('button');claim.type='button';claim.style.cssText='width:100%;margin-top:10px;';claim.textContent='Claim salary';claim.disabled=jobsBusy||available<=0;claim.onclick=()=>jobAction(active,'claim');wrap.append(claim);value.textContent=available>0?`+${formatMM(available)} MM`:percent>=100?'Ready':'0 MM';if(available>0){next.textContent='Salary is ready to collect.';}else{next.textContent=`Next salary in ${formatJobCountdown(target-Date.now())}`;}box.append(wrap);if(jobsProgressTimer)clearInterval(jobsProgressTimer);jobsProgressTimer=setInterval(()=>{const now=Date.now();const elapsed=Math.max(0,now-last);const livePercent=Math.max(percent,Math.min(100,elapsed/(cooldown*1000)*100));fill.style.width=`${livePercent}%`;const remaining=target-now;if(remaining<=0){next.textContent='Salary is ready to collect.';value.textContent=available>0?`+${formatMM(available)} MM`:'Ready';claim.disabled=jobsBusy;fill.style.width='100%';if(!window.mmJobsReady){window.mmJobsReady=true;window.dispatchEvent(new CustomEvent('make-money-job-ready'));}}else{next.textContent=`Next salary in ${formatJobCountdown(remaining)}`;if(!available)window.mmJobsReady=false;}},1000);}
function renderJobs(){const list=$('jobsList');if(!list)return;list.replaceChildren();if(jobsProgressTimer){clearInterval(jobsProgressTimer);jobsProgressTimer=null;}const active=jobs.find(j=>j.active),activeBox=$('activeJob');if(active){activeBox.hidden=false;activeBox.replaceChildren();const strong=document.createElement('strong');strong.textContent=`Active job: ${active.name}`;const small=document.createElement('small');small.textContent=`${formatMM(active.income_per_hour)} MM/hour · ${formatMM(active.daily_cap)} MM/day maximum`;activeBox.append(strong,small);renderJobProgress(active,activeBox);}else{activeBox.hidden=true;window.mmJobsReady=false;}for(const job of jobs){const row=document.createElement('div');row.className=`job-row${job.active?' active':''}`;const head=document.createElement('div');head.className='job-head';const name=document.createElement('span');name.className='job-name';name.textContent=job.name;const rate=document.createElement('span');rate.className='job-rate';rate.textContent=`${formatMM(job.income_per_hour)}/h`;head.append(name,rate);const meta=document.createElement('div');meta.className='job-meta';const cost=document.createElement('span');cost.textContent=job.unlocked?`Training complete · ${formatMM(job.earned_today)} / ${formatMM(job.daily_cap)} today`:`Training: ${formatMM(job.training_cost)} MM`;const cap=document.createElement('span');cap.textContent=`Cap ${formatMM(job.daily_cap)}/day`;meta.append(cost,cap);row.append(head,meta);const button=document.createElement('button');button.type='button';if(!job.unlocked){button.textContent=job.training_cost>0?`Train · ${formatMM(job.training_cost)} MM`:'Unlock free';button.disabled=jobsBusy||balance<job.training_cost;button.onclick=()=>jobAction(job,'unlock');}else if(job.active){button.textContent='Active';button.disabled=true;button.className='secondary';}else{button.textContent='Choose job';button.disabled=jobsBusy;button.className='secondary';button.onclick=()=>jobAction(job,'choose');}row.append(button);list.append(row);}}
async function loadJobs(showLoading=true){if(!sessionValid())return;if(showLoading)$('jobsStatus').textContent='Loading jobs securely...';try{const r=await apiFetch(JOBS_API,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${sessionToken}`},body:JSON.stringify({action:'list'})});const data=await r.json().catch(()=>null);if(!r.ok||!data?.ok)throw new Error(data?.error||`HTTP ${r.status}`);jobs=Array.isArray(data.jobs)?data.jobs.map(j=>({...j,training_cost:Number(j.training_cost||0),income_per_hour:Number(j.income_per_hour||0),daily_cap:Number(j.daily_cap||0),earned_today:Number(j.earned_today||0),cooldown_seconds:Math.max(3600,Number(j.cooldown_seconds||3600)),unlocked:Boolean(j.unlocked),active:Boolean(j.active)})):[];jobProgress=data.progress?{...data.progress,cooldown_seconds:3600,available_salary:Number(data.progress.available_salary||0),progress_percent:Number(data.progress.progress_percent||0)}:null;window.mmJobsReady=Boolean(jobProgress&&Number(jobProgress.available_salary)>0);$('jobsStatus').textContent=jobs.length?'Choose one job. Only one job can generate salary at a time.':'No jobs available.';renderJobs();scheduleJobsRefresh();}catch(e){window.mmJobsReady=false;$('jobsStatus').textContent=e.name==='AbortError'?'Jobs request timed out.':'Jobs unavailable.';}}
async function jobAction(job,action){if(jobsBusy||!sessionValid())return;jobsBusy=true;renderJobs();$('jobsStatus').textContent=action==='unlock'?`Training for ${job.name}...`:action==='choose'?`Switching to ${job.name}...`:'Claiming salary...';try{const r=await apiFetch(JOBS_API,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${sessionToken}`},body:JSON.stringify({action,job_id:job.job_id,operation_key:randomOperationKey()})});const data=await r.json().catch(()=>null);if(!r.ok||!data?.ok)throw new Error(data?.error||`HTTP ${r.status}`);setBalance(data.balance);$('jobsStatus').textContent=action==='unlock'?'Training completed.':action==='choose'?`${job.name} is now your active job.`:`Salary credited: +${formatMM(data.reward)} MM.`;window.dispatchEvent(new CustomEvent('make-money-achievement-refresh'));await loadJobs(false);}catch(e){const messages={INSUFFICIENT_BALANCE:'Not enough MM.',JOB_LOCKED:'Complete the training first.',ALREADY_UNLOCKED:'Already trained.',JOB_NOT_ACTIVE:'This job is not active.',JOB_COOLDOWN:'Salary is not ready yet.',DAILY_LIMIT_REACHED:'Daily salary limit reached.',OPERATION_REUSED:'This operation was already processed.',SESSION_EXPIRED:'Session expired. Reopen the Mini App.'};$('jobsStatus').textContent=e.name==='AbortError'?'Request timed out.':messages[e.message]||`Job action failed (${e.message}).`;}finally{jobsBusy=false;renderJobs();}}
function clearRankingTimer(){if(rankingTimer){clearTimeout(rankingTimer);rankingTimer=null;}}function scheduleRankingRefresh(ms=60000){clearRankingTimer();rankingTimer=setTimeout(()=>loadRanking(false),ms);}
