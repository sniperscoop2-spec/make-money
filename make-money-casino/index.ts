...
async function rpc(name: string, args: Record<string, unknown>) {
  ...
  if (!response.ok) throw new Error(text || `rpc_${response.status}`);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!Number.isInteger(bet) || bet < 10 || bet > 100) {
      return json({ ok: false, error: "INVALID_BET" }, 400);
    }
    if (choice !== "red" && choice !== "black") {
      return json({ ok: false, error: "INVALID_CHOICE" }, 400);
    }
    ...
    const result = await rpc("make_money_casino_roulette", {
    ...
    return json({
      ok: true,
      game: "roulette",
      result_number: Number(row?.result_number ?? 0),
      result_color: row?.result_color ?? "green",
      won: Boolean(row?.won),
      payout: Number(row?.payout ?? 0),
      net_change: Number(row?.net_change ?? 0),
      balance: Number(row?.balance ?? 0),
      wagered_today: Number(row?.wagered_today ?? 0),
      daily_wager_limit: Number(row?.daily_wager_limit ?? 1000),
      house_edge: 1 / 37,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    if (message.includes("invalid_or_expired_session")) {
      return json({ ok: false, error: "SESSION_EXPIRED" }, 401);
    }
    if (message.includes("invalid_session")) {
      return json({ ok: false, error: "UNAUTHORIZED" }, 401);
    }
    if (message.includes("invalid_operation_key")) {
      return json({ ok: false, error: "INVALID_OPERATION_KEY" }, 400);
    }
    ...
    if (message.includes("daily_wager_limit_reached")) {
      return json({ ok: false, error: "DAILY_WAGER_LIMIT_REACHED" }, 409);
    }
    if (message.includes("player_not_found")) {
      return json({ ok: false, error: "PLAYER_NOT_FOUND" }, 404);
    }
    console.error("make-money-casino error", message);
    return json({ ok: false, error: "CASINO_SERVER_ERROR" }, 500);
  }
});
